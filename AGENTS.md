...
vid-studio 是一个 Electron 桌面应用，将 B 站视频转换为结构化知识时间轴（Markdown 格式）。前端为 Electron + Vue 3 + TypeScript，后端为 Python 处理管道（位于 `vid-engine/`，有独立的 `AGENTS.md`）。
